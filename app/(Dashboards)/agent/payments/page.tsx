...
import { Payments as AllPayments } from '../agentContext';


export default function Payments() {
  const [search, setSearch] = useState('');
  const allPayments = useContext(AllPayments)

  const filteredPayments = allPayments && allPayments.filter((payment) => {
          ...
          <table className="w-full text-[10px] sm:text-sm rounded-lg overflow-hidden">
            ...
            <tbody>
              {filteredPayments && filteredPayments.length === 0 ? (
                ...
                filteredPayments && filteredPayments.map((payment, idx) => (
                  <tr
                    key={idx}
                    className={`
                  ...
                  >
                    ...
                    <td className="py-2 px-3 text-green-600">{payment.status}</td>
